...
import { ViewMode, Shift, Employee, DbShiftDefinition, DbStaffAvailability, ShiftStatus, DbClosedDate, StaffGroup } from './types';
...
const App: React.FC = () => {
  ...
  // Anything that changes underneath us, including a pull from another machine,
  // reloads the roster so the screen shows what was actually saved.
  useEffect(() => {
    return db.onChange(async () => {
      const [rosterData, closed, staffData] = await Promise.all([
        db.getShifts(),
        db.getClosedDates(),
        db.getEmployees()
      ]);
  ...
  const handleReorderStaff = async (orderedIds: string[], groups: Record<string, StaffGroup>) => {
    try {
      await db.setStaffOrder(orderedIds, groups);
    } catch (e) {
      console.error(e);
      alert('Failed to save the new staff order');
    }
  };
